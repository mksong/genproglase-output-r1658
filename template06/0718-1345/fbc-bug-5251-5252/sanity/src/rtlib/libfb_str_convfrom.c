...
{ 
  ...
  if (len >= 2) {
    if ((int )*(p + 0) == 38) {
      ...
      if (radix != 0) {
        tmp = fb_hStrRadix2Longint(p + 2, len - 2, radix);
        return ((double )tmp);
      } else {
  ...
  *(q + len) = (char )'\000';
  ret = strtod((char const   */* __restrict  */)q,
...
{ 
  ...
  if ((unsigned int )str->data == (unsigned int )((void *)0)) {
    val = 0.0;
  } else
  if (((unsigned long )str->len & 2147483647UL) == 0UL) {
    val = 0.0;
  } else {
    val = fb_hStr2Double(str->data,
                         (int )((unsigned long )str->len & 2147483647UL));
  }
  fb_hStrDelTemp(str);
  return (val);
}
