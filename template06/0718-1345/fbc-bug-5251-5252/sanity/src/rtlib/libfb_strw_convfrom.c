...
double fb_WstrToDouble(wchar_t const   *src , int len ) ;
...
double fb_WstrVal(wchar_t const   *str ) ;
...
{ 
  ...
  tmp___4 = malloc((unsigned int )(len + 1) * sizeof(wchar_t ));
  q = (wchar_t *)tmp___4;
  i = 0;
  while (i < len) {
    c = (wchar_t )*(p + i);
  ...
  *(q + len) = 0L;
  ret = wcstod((wchar_t const   */* __restrict  */)q,
               (wchar_t **/* __restrict  */)((void *)0));
  free((void *)q);
  return (ret);
}
}
