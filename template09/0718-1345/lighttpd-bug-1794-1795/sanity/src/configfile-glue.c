...
int config_append_cond_match_buffer(connection *con , data_config *dc ,
                                    buffer *buf , int n ) ;
...
extern void configparser(void *yyp , int yymajor , buffer *yyminor ,
                         config_t *ctx ) ;
...
extern data_unset *configparser_merge_data(data_unset *op1 ,
                                           data_unset const   *op2 ) ;
...
{ 
  ...
  while ((cv + i)->key) {
    ...
    switch ((unsigned int const   )(cv + i)->type) {
    case 5U: 
      ...
      while (j < (da->value)->used) {
        if ((unsigned int )(*((da->value)->data + j))->type == 1U) {
          ...
          log_error_write((void *)srv, "configfile-glue.c", 62U, "sssd",
                          ...
                          (cv + i)->key, "type:",
                          (unsigned int )(*((da->value)->data + j))->type);
          return (-1);
        }
        j ++;
      }
...
static cond_result_t config_check_cond_cached(server *srv , connection *con ,
                                              data_config *dc ) ;
static cond_result_t config_check_cond_nocache(server *srv , connection *con ,
                                               data_config *dc ) 
{ 
  buffer *l ;
  ...
  if (dc->parent) {
    if ((dc->parent)->context_ndx) {
      if (con->conf.log_condition_handling) {
        log_error_write((void *)srv, "configfile-glue.c", 220U, "sb",
                        "go parent", (dc->parent)->key);
      } else {
  ...
  if (dc->prev) {
    if (con->conf.log_condition_handling) {
      log_error_write((void *)srv, "configfile-glue.c", 229U, "sb", "go prev",
                      (dc->prev)->key);
    } else {
  ...
  if (! con->conditional_is_valid[dc->comp]) {
    ...
    log_trace("%s.%d: (trace) cond[%d] is valid: %d", tmp___0, 240,
              (unsigned int )dc->comp, con->conditional_is_valid[dc->comp]);
    return ((cond_result_t )0);
  } else {

  }
  switch ((unsigned int )dc->comp) {
  case 3U: 
  ...
  if (tmp___6) {
    ...
    if (! ck_colon) {
      buffer_copy_string_len(srv->cond_check_buf, (char const   *)l->ptr,
                             (size_t )(val_colon - l->ptr));
      l = srv->cond_check_buf;
    } else {
  ...
  if ((unsigned int )dc->cond == 3U) {
    ...
    if ((int )con->dst_addr.plain.sa_family == 2) {
      ...
      if ((unsigned int )((void *)0) != (unsigned int )nm_slash) {
        if (con->conf.log_condition_handling) {
          l = srv->empty_string;
          log_error_write((void *)srv, "configfile-glue.c", 312U, "bsbsb",
                          dc->comp_key, "(", l, ") compare to", dc->string);
        } else {

        }
        if ((int )*(nm_slash + 1) == 0) {
          log_error_write((void *)srv, "configfile-glue.c", 317U, "sb",
                          "ERROR: no number after / ", dc->string);
          return ((cond_result_t )1);
        } else {

        }
        tmp___7 = strtol((char const   */* __restrict  */)(nm_slash + 1),
                         (char **/* __restrict  */)(& err), 10);
        nm_bits = (int )tmp___7;
        if (*err) {
          log_error_write((void *)srv, "configfile-glue.c", 325U, "sbs",
                          "ERROR: non-digit found in netmask:", dc->string,
                          (int )*err);
          return ((cond_result_t )1);
        } else {
        ...
        buffer_copy_string_len(srv->cond_check_buf,
                               (char const   *)(dc->string)->ptr,
                               (size_t )(nm_slash - (dc->string)->ptr));
        tmp___8 = inet_aton((char const   *)(srv->cond_check_buf)->ptr,
                            & val_inp);
        if (0 == tmp___8) {
          log_error_write((void *)srv, "configfile-glue.c", 341U, "sb",
  ...
  break;
  case 4U: 
  tmp___13 = array_get_element(con->request.headers, "Referer",
                               sizeof("Referer") - 1U);
  ...
  break;
  case 6U: 
  tmp___14 = array_get_element(con->request.headers, "Cookie",
                               sizeof("Cookie") - 1U);
  ...
  break;
  case 5U: 
  tmp___15 = array_get_element(con->request.headers, "User-Agent",
  ...
  if ((unsigned int )((void *)0) == (unsigned int )l) {
    if (con->conf.log_condition_handling) {
      log_error_write((void *)srv, "configfile-glue.c", 420U, "bsbs",
  ...
  if (con->conf.log_condition_handling) {
    ...
    log_trace("%s.%d: (trace) \'%s\': \'%s\' is matched against \'%s\'",
              tmp___17, 430, (dc->comp_key)->ptr, l->ptr, (dc->string)->ptr);
  } else {
  ...
  if (tmp___20) {
    ...
    return ((cond_result_t )tmp___19);
  }
  break;
  case 2U: 
  case 4U: 
  cache = con->cond_cache + dc->context_ndx;
  n = pcre_exec((pcre const   *)dc->regex,
                (pcre_extra const   *)dc->regex_study, (char const   *)l->ptr,
                (int )(l->used - 1U), 0, 0, cache->matches,
                (int )(sizeof(cache->matches) / sizeof(cache->matches[0])));
...
{ 
  ...
  if (0U == (unsigned int )(caches + dc->context_ndx)->result) {
    if (con->conf.log_condition_handling) {
      ...
      log_trace("%s.%d: (trace) === start of %d condition block ===", tmp, 478,
                dc->context_ndx);
    } else {
    ...
    if (2U == (unsigned int )tmp___1) {
      if (dc->next) {
        if (con->conf.log_condition_handling) {
          tmp___0 = remove_path("configfile-glue.c");
          log_trace("%s.%d: (trace) setting remains of chaining to %s", tmp___0,
                    484, "false");
        } else {
        ...
        c = dc->next;
        while (c) {
          (caches + c->context_ndx)->result = (cond_result_t )1;
          c = c->next;
        }
      } else {
    ...
    if (con->conf.log_condition_handling) {
      ...
      log_trace("%s.%d: (trace) [%d] result: %s", tmp___3, 494, dc->context_ndx,
                tmp___2);
    } else {
  ...
  if (con->conf.log_condition_cache_handling) {
    ...
    log_trace("%s.%d: (trace) [%d] (cached) result: %s", tmp___5, 500,
              dc->context_ndx, tmp___4);
  } else {
...
{ 
  ...
  {
  memset((void *)con->cond_cache, 0,
         sizeof(cond_cache_t ) * (srv->config_context)->used);
  i = (size_t )0;
  while (i < 11U) {
...
{ 
  ...
  buffer_append_string_len(buf,
                           (char const   *)((cache->comp_value)->ptr + cache->matches[n]),
                           (size_t )(cache->matches[n + 1] - cache->matches[n]));
  return (1);
}
}
int config_exec_pcre_keyvalue_buffer(connection *con ,
