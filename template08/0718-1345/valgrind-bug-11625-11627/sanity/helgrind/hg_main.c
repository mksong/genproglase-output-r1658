...
enum __anonenum_Vg_TCheckClientRequest_97 {
    ...
    _VG_USERREQ__HG_USERSO_FORGET_ALL = 1212612899,
...
extern Thread *libhb_get_Thr_hgthread(Thr * ) ;
extern void libhb_set_Thr_hgthread(Thr * , Thread * ) ;
...
{ 
  ...
  if ((unsigned int )lk == (unsigned int )admin_locks) {
    if ((unsigned int )lk->admin_prev == (unsigned int )((void *)0)) {

    } else {
      vgPlain_assert_fail((Bool )0, (Char const   *)"lk->admin_prev == NULL",
                          (Char const   *)"hg_main.c", 204,
                          (Char const   *)"del_LockN", "");
    }
    if (lk->admin_next) {
      (lk->admin_next)->admin_prev = (struct _Lock *)((void *)0);
    } else {
...
{ 
  ...
  {
  map_usertag_to_SO_INIT();
  tmp = vgPlain_delFromFM(map_usertag_to_SO, & keyW, & valW, usertag);
  if (tmp) {
    so = (SO *)valW;
    if (keyW == usertag) {

    } else {
      vgPlain_assert_fail((Bool )0, (Char const   *)"keyW == usertag",
                          (Char const   *)"hg_main.c", 3029,
                          (Char const   *)"map_usertag_to_SO_delete", "");
    }
    if (so) {

    } else {
      vgPlain_assert_fail((Bool )0, (Char const   *)"so",
                          (Char const   *)"hg_main.c", 3030,
                          (Char const   *)"map_usertag_to_SO_delete", "");
    }
    libhb_so_dealloc(so);
  } else {
...
{ 
  ...
  if (so) {
    ...
    vgPlain_assert_fail((Bool )0, (Char const   *)"so",
                        (Char const   *)"hg_main.c", 3059,
                        (Char const   *)"evh__HG_USERSO_SEND_PRE", "");
  }
  libhb_so_send(thr->hbthr, so, (Bool )0);
  return;
}
}
...
static void evh__HG_USERSO_FORGET_ALL(ThreadId tid , UWord usertag ) 
{ 
...
{ 
  ...
  if (vgPlain_clo_stats) {
    ...
    if (vgHelgrind_clo_track_lockorders) {
      tmp___0 = vgHelgrind_cardinalityWSU(univ_laog);
      vgPlain_printf("       univ_laog: %\'8d unique lock sets\n", (Int )tmp___0);
    } else {

    }
    tmp___1 = vgHelgrind_stats__LockN_to_P_get_map_size();
    vgPlain_printf("  LockN-to-P map: %\'8llu queries (%llu map size)\n",
                   vgHelgrind_stats__LockN_to_P_queries, tmp___1);
    tmp___2 = vgHelgrind_stats__string_table_get_map_size();
    vgPlain_printf("string table map: %\'8llu queries (%llu map size)\n",
                   vgHelgrind_stats__string_table_queries, tmp___2);
    if (vgHelgrind_clo_track_lockorders) {
      if (laog) {
        tmp___3 = vgPlain_sizeFM(laog);
        tmp___4 = tmp___3;
      } else {
        tmp___4 = (UWord )0;
      }
      ...
      if (laog_exposition) {
        tmp___5 = vgPlain_sizeFM(laog_exposition);
        tmp___6 = tmp___5;
      } else {
        tmp___6 = (UWord )0;
      }
...
{ 
  ...
  if (thr) {
    ...
    vgPlain_assert_fail((Bool )0, (Char const   *)"thr",
                        (Char const   *)"hg_main.c", 4709,
                        (Char const   *)"for_libhb__get_stacktrace", "");
  }
  tid = map_threads_maybe_reverse_lookup_SLOW(thr);
  tmp = vgPlain_get_StackTrace(tid, frames, (UInt )nRequest,
                               (StackTrace )((void *)0),
