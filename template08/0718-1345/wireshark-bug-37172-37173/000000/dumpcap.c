...
extern gboolean cf_find_packet_time_reference(capture_file *cf ,
                                              search_direction dir ) ;
extern gboolean cf_goto_top_frame() ;
extern gboolean cf_goto_bottom_frame() ;
...
static void capture_loop_packet_cb(u_char *user ,
                                   struct pcap_pkthdr  const  *phdr ,
                                   u_char const   *pd ) ;
...
static pcap_t *open_capture_device(interface_options *interface_opts ,
                                   capture_options *capture_opts  __attribute__((__unused__)) ,
...
{ 
  ...
  if (byte_swapped) {
    {
    memset(rechdr, 0, sizeof(struct pcaprec_hdr ));
...
{ 
  ...
  capture_loop_packet_cb((u_char *)ld, (struct pcap_pkthdr  const  *)(& phdr),
                         (u_char const   *)data);
  pcap_opts->cap_pipe_state = (enum __anonenum_cap_pipe_state_168 )0;
  return (1);
  case 2U: 
...
{ 
  ...
  while (i < (capture_opts->ifaces)->len) {
    ...
    g_log("CaptureChild", (GLogLevelFlags )128, "capture_loop_open_input : %s",
          interface_opts.name);
    pcap_opts.pcap_h = open_capture_device(& interface_opts, capture_opts,
                                           & open_err_str);
...
static int capture_loop_dispatch(capture_options *capture_opts  __attribute__((__unused__)) ,
                                 loop_data *ld , char *errmsg , int errmsg_len ,
                                 pcap_options *pcap_opts ) 
{ 
  int inpkts ;
  ...
  *stats_known = 0;
  ...
  while (global_ld.go) {
    inpkts = capture_loop_dispatch(capture_opts, & global_ld, errmsg,
                                   (int )sizeof(errmsg), & pcap_opts);
    if (inpkts > 0) {
      global_ld.inpkts_to_sync_pipe += inpkts;
      if ((unsigned int )cnd_autostop_size != (unsigned int )((void *)0)) {
        tmp___6 = cnd_eval(cnd_autostop_size, (guint32 )global_ld.bytes_written);
        if (tmp___6) {
          tmp___5 = do_file_switch_or_stop(capture_opts, cnd_autostop_files,
    ...
    if (cur_time.tv_sec * 1000000L + cur_time.tv_usec > (upd_time.tv_sec * 1000000L + upd_time.tv_usec) + 500000L) {
      ...
      if ((unsigned int )cnd_autostop_duration != (unsigned int )((void *)0)) {
        tmp___7 = cnd_eval(cnd_autostop_duration);
        if (tmp___7) {
          global_ld.go = 0;
          continue;
        } else {
      ...
      if ((unsigned int )cnd_file_duration != (unsigned int )((void *)0)) {
        tmp___9 = cnd_eval(cnd_file_duration);
        if (tmp___9) {
          tmp___8 = do_file_switch_or_stop(capture_opts, cnd_autostop_files,
                                           cnd_autostop_size, cnd_file_duration);
          if (tmp___8) {
  ...
  while (i < (capture_opts->ifaces)->len) {
    ...
    if (pcap_opts.pcap_err) {
      ...
      if (0) {
        ...
        if (tmp___16) {
          tmp___11 = __builtin_strcmp((char const   *)cap_err_str,
      ...
      if (tmp___15 == 0) {
        ...
        if (0) {
          ...
          if (tmp___23) {
            tmp___18 = __builtin_strcmp((char const   *)cap_err_str,
        ...
        if (tmp___22 == 0) {
          ...
          if (0) {
            ...
            if (tmp___30) {
              tmp___25 = __builtin_strcmp((char const   *)cap_err_str,
          ...
          if (tmp___29 == 0) {
            ...
            if (0) {
              ...
              if (tmp___37) {
                tmp___32 = __builtin_strcmp((char const   *)cap_err_str,
                                            "read: I/O error");
                tmp___36 = tmp___32;
              } else {
            ...
            if (tmp___36 == 0) {
              ...
              if (0) {
                ...
                if (tmp___44) {
                  tmp___39 = __builtin_strcmp((char const   *)cap_err_str,
  ...
  while (i < (capture_opts->ifaces)->len) {
    ...
    if ((unsigned int )pcap_opts.pcap_h != (unsigned int )((void *)0)) {
      while (1) {
        ...
        tmp___45 = __builtin_expect((long )_g_boolean_var_, 1L);
        if (tmp___45) {

        } else {
          g_assertion_message_expr((char const   *)((gchar *)0), "dumpcap.c",
                                   3262, "capture_loop_start",
                                   "!pcap_opts.from_cap_pipe");
        }
        break;
      }
      tmp___47 = pcap_stats(pcap_opts.pcap_h, stats);
      if (tmp___47 >= 0) {
        *stats_known = 1;
        report_packet_drops(stats->ps_recv, stats->ps_drop, interface_opts.name);
      } else {
        tmp___46 = pcap_geterr(pcap_opts.pcap_h);
        g_snprintf(errmsg, (gulong )sizeof(errmsg),
                   "Can\'t get packet-drop statistics: %s", tmp___46);
        report_capture_error((char const   *)(errmsg), please_report);
      }
    } else {
...
{ 
  ...
  if (ld->pdh) {
    if (global_capture_opts.use_pcapng) {
      successful = libpcap_write_enhanced_packet_block(ld->pdh, phdr,
                                                       (guint32 )0, pd,
                                                       & ld->bytes_written,
                                                       & err);
    } else {
      successful = libpcap_write_packet(ld->pdh, phdr, pd, & ld->bytes_written,
                                        & err);
    }
    if (! successful) {
      ld->go = 0;
      ...
      (ld->packet_count) ++;
      if (ld->packet_max > 0) {
        if (ld->packet_count >= ld->packet_max) {
          ld->go = 0;
        } else {
...
{ 
  ...
  print_statistics = 0;
  run_once_args = 0;
  debug_log = fopen((char const   */* __restrict  */)"dumpcap_debug_log.tmp",
                    (char const   */* __restrict  */)"w");
  if ((unsigned int )debug_log == (unsigned int )((void *)0)) {
    fprintf((FILE */* __restrict  */)stderr,
            (char const   */* __restrict  */)"Unable to open debug log file !\n");
    exit(1);
  } else {
  ...
  while (1) {
    opt = getopt(argc, (char * const  *)argv, "a:b:B:c:dDf:ghi:ILMnpqSs:vw:y:Z:");
    if (opt != -1) {
...
{ 
  ...
  while (1) {
    g_assertion_message((char const   *)((gchar *)0), "dumpcap.c", 4079,
  ...
  if (! (((int )log_level & -4) & -193)) {
    ...
    fflush(stderr);
    fprintf((FILE */* __restrict  */)debug_log,
            (char const   */* __restrict  */)"%s", msg);
    fflush(debug_log);
    g_free((gpointer )msg);
    return;
